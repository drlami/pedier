...
function DrugCard({ drug, weightKg, pmaWeeks, pnaDays, highlighted }: {
  drug: NeonateDrug;
  weightKg: number;
  pmaWeeks: number;
  pnaDays: number;
  ...
  const pinned = isPinned({ type: "drug", system: "neodose", id: drug.id });
  ...
  return (
    <div ref={cardRef} className={cn(
      ...
      >
        ...
        <button
          onClick={(e) => { e.stopPropagation(); togglePin({ type: "drug", system: "neodose", id: drug.id }); }}
          className={cn(
      ...
      {expanded && (
        <div className="px-4 pb-4 border-t mt-0 space-y-4">
          ...
          {drug.levelGuidance && <LevelGuidanceSection guidance={drug.levelGuidance} />}
