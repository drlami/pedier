...
export default function PICUDashboard() {
  ...
  const picuProtocols = useMemo(() => {
    return allProtocols.filter(p => p.unit === "picu");
  }, [allProtocols]);
  ...
  const searchResults = useMemo(() => {
    ...
    return {
      protocols: picuProtocols.filter(p => 
  ...
  }, [searchTerm, picuProtocols]);

  const systems = useMemo(() => {
    const set = new Set(picuProtocols.map(p => p.system));
    return Array.from(set).sort((a, b) => a.localeCompare(b));
  }, [picuProtocols]);

  const systemProtocols = useMemo(() => {
    if (!selectedSystem) return [];
    return picuProtocols
      .filter(p => p.system === selectedSystem)
      .sort((a, b) => a.name.localeCompare(b.name));
  }, [selectedSystem, picuProtocols]);

  return (
    <div className="max-w-4xl mx-auto space-y-10 pb-32 px-2 sm:px-4">
      {/* 1. EMERGENCY HERO */}
      <section className="grid grid-cols-1 md:grid-cols-12 gap-4">
        <Link href="/cardiac-arrest" className="md:col-span-5 group relative overflow-hidden rounded-[32px] bg-purple-600 p-6 text-white shadow-xl transition-all hover:scale-[1.02] active:scale-[0.98]">
          <div className="relative z-10 h-full flex flex-col justify-between min-h-[140px]">
            ...
            <div>
              ...
              <p className="text-purple-100 text-sm font-medium mt-2">Drugs • Equipment • RSI</p>
            </div>
          </div>
        ...
        <div className="md:col-span-7 flex flex-col justify-center space-y-4 p-2">
          <div className="space-y-1">
            <h2 className="text-2xl font-black tracking-tight text-foreground">PICU Dashboard</h2>
            <p className="text-muted-foreground text-sm font-medium">Search PICU protocols, calculators, and clinical tools.</p>
          </div>
      ...
      {searchTerm.trim() ? (
        ...
        <section className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
          <div className="flex items-center justify-between px-2">
            ...
            <Button variant="ghost" size="sm" onClick={() => setLocation("/picu")} className="text-xs font-bold text-muted-foreground">
        ...
        <>
          ...
          <section className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
              {systems.map(system => {
                const count = picuProtocols.filter(p => p.system === system).length;
                return (
                  <button 
                    key={system}
                    onClick={() => setLocation(`/picu?system=${encodeURIComponent(system)}`)}
