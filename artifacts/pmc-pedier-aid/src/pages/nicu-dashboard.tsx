...
export default function NICUDashboard() {
  ...
  const nicuProtocols = useMemo(() => allProtocols.filter((p) => p.unit === "nicu"), [allProtocols]);
  ...
  const searchResults = useMemo(() => {
    ...
    return {
      protocols: nicuProtocols
        .filter((p) => p.name.toLowerCase().includes(q) || p.system.toLowerCase().includes(q))
        .sort((a, b) => a.name.localeCompare(b.name)),
      calculators: CALCULATOR_SHORTCUTS.filter((c) => c.label.toLowerCase().includes(q)),
    };
  }, [searchTerm, nicuProtocols]);

  const systems = useMemo(() => {
    const set = new Set(nicuProtocols.map((p) => p.system));
    return Array.from(set).sort((a, b) => a.localeCompare(b));
  }, [nicuProtocols]);

  const systemProtocols = useMemo(() => {
    if (!selectedSystem) return [];
    return nicuProtocols.filter((p) => p.system === selectedSystem).sort((a, b) => a.name.localeCompare(b.name));
  }, [selectedSystem, nicuProtocols]);

  return (
    <div className="max-w-4xl mx-auto space-y-10 pb-32 px-2 sm:px-4">
      {/* HERO */}
      <section className="grid grid-cols-1 md:grid-cols-12 gap-4">
        <Link href="/nicu/drugs" className="md:col-span-5 group relative overflow-hidden rounded-[32px] bg-teal-600 p-6 text-white shadow-xl transition-all hover:scale-[1.02] active:scale-[0.98]">
          <div className="relative z-10 h-full flex flex-col justify-between min-h-[140px]">
            <div className="flex items-center gap-3">
              <div className="p-2.5 rounded-2xl bg-white/20 backdrop-blur-md">
                <Pill className="h-7 w-7 text-white" />
              </div>
              <Badge className="bg-white/20 text-white border-none font-black tracking-widest text-[10px]">NEODOSE</Badge>
            </div>
            <div>
              <h1 className="text-4xl font-black tracking-tighter leading-none">DRUGS</h1>
              <p className="text-teal-100 text-sm font-medium mt-2">Neonatal IV dosing by weight + PMA/PNA</p>
            </div>
          </div>
        ...
        <div className="md:col-span-7 flex flex-col justify-center space-y-4 p-2">
          <div className="space-y-1">
            <h2 className="text-2xl font-black tracking-tight text-foreground">NICU Dashboard</h2>
            <p className="text-muted-foreground text-sm font-medium">Search neonatal protocols, calculators, and the NeoDose drug reference.</p>
          </div>
          <div className="relative group">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground/40 group-focus-within:text-primary transition-colors" />
            <Input
              type="search"
              placeholder="Search e.g. RDS, Sepsis, Gentamicin..."
      ...
      {searchTerm.trim() ? (
            ...
            {searchResults.calculators.map((calc) => (
              <div key={calc.href} className="group flex items-center justify-between p-4 rounded-2xl border bg-card hover:border-primary/20 transition-all">
                <Link href={calc.href} className="flex items-center gap-4 flex-1">
                  <div className="p-2.5 rounded-xl bg-orange-50 text-orange-600"><Calculator className="h-5 w-5" /></div>
            ...
            {searchResults.protocols.map((p) => (
              <div key={p.id} className="group flex items-center justify-between p-4 rounded-2xl border bg-card hover:border-primary/20 transition-all">
                <Link href={`/diseases/${p.id}`} className="flex items-center gap-4 flex-1">
                  <div className="p-2.5 rounded-xl bg-teal-50 text-teal-600"><BookOpen className="h-5 w-5" /></div>
                  <div>
        ...
        <section className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
          <div className="flex items-center justify-between px-2">
            <div className="flex items-center gap-3">
              <div className="p-2 rounded-lg bg-primary/10 text-primary"><Stethoscope className="h-4 w-4" /></div>
              <h3 className="text-xl font-black tracking-tight">{selectedSystem}</h3>
            </div>
            <Button variant="ghost" size="sm" onClick={() => setLocation("/nicu")} className="text-xs font-bold text-muted-foreground">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {systemProtocols.map((p) => (
              <div key={p.id} className="group flex items-center justify-between p-4 rounded-2xl border bg-card hover:border-primary/20 transition-all">
                <Link href={`/diseases/${p.id}`} className="flex items-center gap-4 flex-1">
                  <div className="p-2 rounded-full bg-primary/5 text-primary opacity-40 group-hover:opacity-100 transition-opacity"><ChevronRight className="h-4 w-4" /></div>
        ...
        <>
          <section className="space-y-6">
            ...
            {systems.length === 0 ? (
              <div className="p-8 text-center rounded-2xl border-2 border-dashed text-sm font-bold text-muted-foreground">
                Neonatal protocols are being added. Use the NeoDose drug calculator above in the meantime.
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                {systems.map((system) => {
                  const count = nicuProtocols.filter((p) => p.system === system).length;
                  return (
                    <button
                      key={system}
                      onClick={() => setLocation(`/nicu?system=${encodeURIComponent(system)}`)}
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <div className="p-2 rounded-xl bg-muted group-hover:bg-primary/10 group-hover:text-primary transition-colors"><BookOpen className="h-4 w-4" /></div>
