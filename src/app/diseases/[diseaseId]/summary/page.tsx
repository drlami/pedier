...
export default function SummaryPage({ params, searchParams }: SummaryPageProps) {
  ...
  // Re-create form data from search params
  ...
  // Re-calculate results on the server
  ...
  const getAnswerDisplay = (id: string, value: any): string => {
    ...
    if (question.type === 'select' || question.type === 'radio') {
      return question.options?.find(o => String(o.value) === String(value))?.label || String(value);
    }
  ...
  return (
    <div className="max-w-4xl mx-auto bg-card p-4 sm:p-6 md:p-8 rounded-lg print-p-0 print-shadow-none">
      <div className="flex justify-between items-start mb-6">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold font-headline text-primary">{protocol.name} - Assessment Summary</h1>
          <p className="text-muted-foreground">Generated on: {new Date().toLocaleString()}</p>
        </div>
        <Button onClick={() => typeof window !== 'undefined' && window.print()} className="no-print">
        ...
        <ResultCard title="Patient Assessment" icon={FileQuestion}>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                {Object.entries(formData).map(([key, value]) => (
                    <div key={key}>
                        <p className="font-semibold text-sm">{protocol.questions.find(q => q.id === key)?.questionText}</p>
                        <p className="text-muted-foreground">{getAnswerDisplay(key, value)}</p>
                    </div>
                ))}
            </div>
        </ResultCard>
